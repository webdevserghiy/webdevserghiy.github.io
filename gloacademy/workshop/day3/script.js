	...
	fetch('https://api.themoviedb.org/3/search/multi?api_key=c0ff64cdede0859442f376546a6f9258&language=ru-RU&query=' + searchText)
		...
		.then(function (output) {
			...
			output.results.forEach(function (item) {
				...
				<div class="film-card">
					<img class="img-poster" src="${item.poster_path ? urlPoster + item.poster_path : 'http://dummyimage.com/500x750.jpg/F3F1ED/000000&text=НЕТ_ПОСТЕРА!'}" alt="${nameItem}" ${dataInfo}>
...
function showFullInfo() {
	...
	fetch(url)
		...
		.then(function (output) {
			...
			<div class="col-4">
				<img class="img" src="${output.poster_path ? urlPoster + output.poster_path : 'http://dummyimage.com/500x750.jpg/F3F1ED/000000&text=НЕТ_ПОСТЕРА!'}" alt="${output.name || output.title}>
